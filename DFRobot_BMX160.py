...
class BMX160:
    ...
    _BMX160_ACCEL_MG_LSB_2G          = (4/65535)
    _BMX160_ACCEL_MG_LSB_4G          = (0.000122070)
    _BMX160_ACCEL_MG_LSB_8G          = (0.000244141)
    _BMX160_ACCEL_MG_LSB_16G         = (0.000488281)
    
    _BMX160_GYRO_SENSITIVITY_125DPS  = ((2**-15) * 125)
    _BMX160_GYRO_SENSITIVITY_250DPS  = ((2**-15) * 250)
    _BMX160_GYRO_SENSITIVITY_500DPS  = ((2**-15) * 500)
    _BMX160_GYRO_SENSITIVITY_1000DPS = ((2**-15) * 1000)
    _BMX160_GYRO_SENSITIVITY_2000DPS = ((2**-15) * 2000)
    ...
    def set_gyro_range(self, bits):
        ...
        if bits == 0:
            ...
            gyro_cmd = self.GyroRange_125DPS
        elif bits == 1:
            self.gyroRange = self._BMX160_GYRO_SENSITIVITY_250DPS
            gyro_cmd = self.GyroRange_250DPS
        elif bits == 2:
            self.gyroRange = self._BMX160_GYRO_SENSITIVITY_500DPS
            gyro_cmd = self.GyroRange_500DPS
        elif bits == 3:
            self.gyroRange = self._BMX160_GYRO_SENSITIVITY_1000DPS
            gyro_cmd = self.GyroRange_1000DPS
        elif bits == 4:
            self.gyroRange = self._BMX160_GYRO_SENSITIVITY_2000DPS
            gyro_cmd = self.GyroRange_2000DPS
        else:
            self.gyroRange = self._BMX160_GYRO_SENSITIVITY_250DPS
            gyro_cmd = self.GyroRange_250DPS
        
        self.write_bmx_reg(self._BMX160_GYRO_RANGE_ADDR, gyro_cmd)
    ...
    def set_accel_range(self, bits):
        ...
        if bits == 0:
            ...
            accel_cmd = self.AccelRange_2G
        elif bits == 1:
            self.accelRange = self._BMX160_ACCEL_MG_LSB_4G
            accel_cmd = self.AccelRange_4G
        elif bits == 2:
            self.accelRange = self._BMX160_ACCEL_MG_LSB_8G
            accel_cmd = self.AccelRange_8G
        elif bits == 3:
            self.accelRange = self._BMX160_ACCEL_MG_LSB_16G
            accel_cmd = self.AccelRange_16G
        else:
            self.accelRange = self._BMX160_ACCEL_MG_LSB_2G
            accel_cmd = self.AccelRange_2G

        self.write_bmx_reg(self._BMX160_ACCEL_RANGE_ADDR, accel_cmd)
